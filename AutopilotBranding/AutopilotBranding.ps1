...
Copy-Item "$installFolder\Skydance.theme" "C:\Windows\Resources\OEM Themes\Skydance.theme" -Force
Mkdir "C:\Windows\web\wallpaper\Autopilot" -Force | Out-Null
Copy-Item "$installFolder\Skydance.jpg" "C:\Windows\web\wallpaper\Autopilot\Skydance.jpg" -Force
Log "Setting Autopilot theme as the new user default"
reg.exe add "HKLM\TempUser\SOFTWARE\Microsoft\Windows\CurrentVersion\Themes" /v InstallTheme /t REG_EXPAND_SZ /d "%SystemRoot%\resources\OEM Themes\Skydance.theme" /f | Out-Host
